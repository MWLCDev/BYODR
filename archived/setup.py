...
""".format(
        **{"idVendor": "{idVendor}", "idProduct": "{idProduct}", "group": APP_GROUP_NAME}
    )
...
"""
    ...
    def __init__(self, script_location, build_dirname="build"):
        self._docker_files = [os.path.join(script_location, "docker", "docker-compose.yml"), os.path.join(script_location, "docker", "docker-compose.tegra.yml")]
...
"""
    ...
    def create_services(self):
        _m = {"sd_service_user": self.get_user(), "sd_service_group": self.get_group(), "sd_compose_files": " ".join("-f {}".format(name) for name in self._docker_files)}
