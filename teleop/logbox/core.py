...
class SharedState(object):
    ...
    def _expire(self, cmd, stamp):
        return (
            None
            if cmd is None or abs(stamp - get_timestamp(cmd)) > self._patience
            else cmd
        )
...
class MongoLogBox(object):
    ...
    def ensure_indexes(self):
        ...
        _create_index_if_not_exists(
            ...
            [
                ...
                ("img_num_bytes", pymongo.ASCENDING),
            ],
            name="idx_non_packaged_save_events",
        )
        _create_index_if_not_exists(
            _coll,
            [
                ("trigger", pymongo.ASCENDING),
                ("lb_is_packaged", pymongo.ASCENDING),
                ("img_num_bytes", pymongo.ASCENDING),
            ],
            name="idx_non_packaged_save_events",
    ...
    def load_event_image_fields(self, object_id):
        ...
        return (
            None if event is None else event.get("img_shape"),
            event.get("img_num_bytes"),
            event.get("img_buffer"),
        )
    ...
    def list_next_batch_of_non_packaged_save_events(self, batch_size=1000):
        ...
        cursor = self._database.events.find(
            filter=_filter, sort=[("time", -1)], batch_size=batch_size, limit=batch_size
        )
        return list(cursor)

    def list_all_non_packaged_photo_events(self):
        _filter = {
            "trigger": TRIGGER_PHOTO_SNAPSHOT,
            "lb_is_packaged": 0,
            "img_num_bytes": {"$gt": 0},
        }
