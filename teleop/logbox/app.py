...
class PackageApplication(Application):
    def __init__(self, mongo, user, event, hz=1e-2, sessions_dir=os.getcwd()):
        ...
        self._mongo = mongo
        self._user = user
        # Folder where training sessions are stored
        # Usually it is /sessions/autopilot/
        # It is accessed through the FTP service to download
        self._recorder_dir = get_or_create_directory(
            os.path.join(sessions_dir, "autopilot")
        )
        self._photo_dir = get_or_create_directory(
            os.path.join(sessions_dir, "photos", "cam0")
        )
    ...
    def _mark(self, item):
        self._mongo.update_event(
            {"_id": item.get("_id")}, {"$set": {"lb_is_packaged": 1}}
        )
        ...
        if not self._user.is_busy(wait_sec=10):
            ...
            if len(items) > 0 and not self._user.is_busy(wait_sec=10):
                _directory = os.path.join(
                    self._photo_dir,
                    datetime.fromtimestamp(items[0].get("time") * 1e-6).strftime(
                        "%Y%B"
                    ),
                )
                ...
                with open(os.path.join(_directory, "photo.log"), "a+") as f:
                    for item in items:
                        ...
                        _dts = datetime.fromtimestamp(_timestamp * 1e-6).strftime(
                            "%Y%b%dT%H%M%S"
                        )
                        latitude = str(item.get("veh_gps_latitude"))[:8].replace(
                            ".", "_"
                        )
                        longitude = str(item.get("veh_gps_longitude"))[:8].replace(
                            ".", "_"
                        )
                        fname = "{}_lat{}_long{}.jpg".format(_dts, latitude, longitude)
                        cv2.imwrite(
                            os.path.join(_directory, fname),
                            cv2_image_from_bytes(item.get("img_buffer")),
                        )
                        f.write(
                            "{} {} latitude {} longitude {}\r\n".format(
                                _timestamp, fname, latitude, longitude
                            )
                        )
    ...
    def _package_next(self):
        ...
        if len(events) < 1:
            return False
        logger.info(
            "Packaging {} valid events out of {} total items.".format(
...
class LogApplication(Application):
    ...
    def step(self):
        ...
        _operator = _operator and (
            bool(pil.get("forced_steering", 0)) or bool(pil.get("forced_throttle", 0))
        )
        ...
        if any(
            [
                cmd.get("button_right", 0) == 1
                for cmd in ([] if pilot_all is None else pilot_all)
            ]
        ):
            self._photos.append(_contents)
        list(
            map(
                lambda x: self._insert(
                    TRIGGER_PHOTO_SNAPSHOT, content=x, save_image=True
                ),
                self._photos.pop_all(),
            )
        )
