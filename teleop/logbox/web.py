...
def _float_or_default(x, default=0.0):
    try:
...
class WebEventViewer(object):
    ...
    def __call__(self, *args, **kwargs):
        _steering_scale = _float_or_default(
            kwargs.get("pil_steering_scale"), default=1.0
        )
        _pil_steering = _float_scaled(kwargs.get("pil_steering"), scale=_steering_scale)
        _inf_steering = _float_scaled(
            kwargs.get("inf_steer_action"), scale=_steering_scale
        )
        return [
            str(kwargs.get("_id")),
            kwargs.get("time"),
...
class DataTableRequestHandler(web.RequestHandler):
    ...
    def get(self):
        ...
        c_total, cursor = self._box.paginate_events(
            start=start, length=length, order=time_order
        )
        data = []
...
class JPEGImageRequestHandler(web.RequestHandler):
    ...
    @tornado.gen.coroutine
    def get(self):
        _fields = self._box.load_event_image_fields(
            self.get_query_argument("object_id")
        )
        _written = False
